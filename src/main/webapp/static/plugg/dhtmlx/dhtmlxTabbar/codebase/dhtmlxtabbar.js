...
dhtmlXTabBar.prototype = {
	...
	setSkin: function(a) {
		...
		var b = a.indexOf("dhx_sky") == 0,
			...
			tdh_b = a.indexOf("tdh_blue") == 0,
			tdh_d = a.indexOf("tdh_default") == 0;
		if(d && !this._custom_height) this._s.line_height = 37, this._setRowSizes();
		if(tdh_b) this._s.line_height = 32, this._setRowSizes();
		if(tdh_d) this._s.line_height = 40, this._setRowSizes();
		if(tdh_b || tdh_d){
			this._s.skin_line = 0;
		...
		if(b) {
			...
			g.className = "dhx_tabbar_lineA";
			this._tabAll.appendChild(g);
			var h = document.createElement("DIV");
			h.className = "dhx_tabbar_lineB";
			this._tabAll.appendChild(h);
			var i = document.createElement("DIV");
			i.className = "dhx_tabbar_lineC";
			this._tabAll.appendChild(i);
			var j = document.createElement("DIV");
			j.className = "dhx_tabbar_lineD";
			this._tabAll.appendChild(j);
			this._getCoverLine();
	...
	_setSizes: function(a, b) {
		...
		var d = _isIE && document.compatMode == "BackCompat",
			...
			g = this._rows.length * (this._s.line_height - (this._s.skin_line_x ? 4 : 0)) + (this._s.skin_line_x ? 2 : 0);
	...
	_setRowSizes: function() {
		for(var a = 0; a < this._rows.length; a++) this._rows[a].style[this._dy] = this._s.line_height + "px", this._rows[a].style[this._py] = a * (this._s.line_height - (this._s.skin_line_x ?
	...
	moveTab: function(a, b) {
		...
		if(c) {
			...
			d != b && (d < b && b++, c.parentNode.childNodes[b] ? c.parentNode.insertBefore(c,
				c.parentNode.childNodes[b]) : c.parentNode.appendChild(c), this._setTabSizes(c.parentNode))
		}
	},
	...
	_createTab: function(a, b, c) {
		...
		this._close && (e += "<IMG style='" + this._pxc + ":4px; " + this._py + (this.skin == "dhx_web" ? ":7" : ":4") + "px; position:absolute;z-index:11;' src='" + this.imgUrl +
			this.skin + "/close.png' >");
		d.innerHTML = e;
	...
	_img_all: function(a) {
		var b = this._getSkin(a),
			c = a._active ? 1 : 4;
		this._img(a.childNodes[1], c, this._px, b.left);
		this._img(a.childNodes[2], c + 2, this._pxc, b.right);
		this._img(a.childNodes[3], c + 1, this._px, parseInt(a.style[this._dx]) - (b.left + b.right), b.left)
	},
	_get_img_pos: function(a) {
		this._s._bMode && a < 7 && (a = Math.abs(a - 6));
		a = -5 - a * (this._s._vMode == 1 && this.skin == "dhx_terrace" ? 245 : 45);
		return this._s._vMode ? a + "px 0px" : "0px " + a + "px"
	},
	_img: function(a, b, c, d, e) {
		if(typeof a !=
			"undefined") {
			if(this.skin == "tdh_blue"){
				var aIndex = $(a).index(), imgType = "tab1_0";
				if($(a).parent().hasClass("dhx_tab_element_active")){ imgType = "tab2_0"; }
				$(a).height(this._s.line_height-1);
				a.style.background = "url(" + this.imgUrl + this.skin + "/" + imgType + aIndex + ".png) no-repeat left bottom";
				if(aIndex = "3"){ a.style.backgroundRepeat = "repeat-x"; }
			}else{
	...
	_addTab: function(a, b, c, d) {
		...
		var e2=e;
		if(this.skin == "tdh_blue" && a.tabCount != 1){
			e2=e-16;
		}
		b.style[this._s.align ? this._pxc : this._px] = e2 + "px";
		d != a.tabCount - 1 && this._setTabSizes(a)
	},
	adjustTabSize: function(a, b) {
		var c = this._getSkin(c);
		if(!b) a.style.fontWeight = "bold", a.childNodes[3].style[this._dx] = a.style[this._dx] = "10px", b = a[this._s._vMode ? "scrollHeight" : "scrollWidth"] + 10 + (this._close ? 20 : 0), a.style.fontWeight = "";
	...
	_onMouseOverHandler: function(a) {
		var b =
			this.tabbar._getTabTarget(a ? a.target : event.srcElement);
	...
	_showHover: function(a) {
		if(a != this._lastHower) {
			if(this._lastHower && this._lastHower != this._lastActive) {
				...
				b.hover ? this._img_all(this._lastHower) : this._img(this._lastHower.childNodes[3], 5, this._px, parseInt(this._lastHower.style[this._dx]) - (b.left +
					b.right), b.left);
	...
	_getTabTarget: function(a) {
		...
		for(; !a.className || a.className.indexOf("dhx_tab_element") == -1;) {
			if(a.className && a.className.indexOf("dhx_tabbar_zone") !=
				-1) return null;
			a = a.parentNode;
			if(!a) return null
		}
	...
	_onClickHandler: function(a) {
		...
		if(c && !c._disabled) this.tabbar._setTabActive(c);
			...
			else if(b.className == "dhx_tab_scroll_right") b.parentNode.scrollLeft += b.parentNode.offsetWidth / 2, d = b.previousSibling,
	...
	_setTabInActive: function(a) {
		...
		a.className = a.className.replace("_active",
			"_inactive");
	...
	_setTabActive: function(a, b) {
		...
		if(b || this.callEvent("onSelect", [c, d])) {
			...
			if(this._c[c]) a.childNodes[0].style.cssText = this._c[c].scolor;
			this._img_all(a);
			this._setTabTop(a);
			this._lastActive = a;
			this.cells(c).show();
			...
			if(this._rows[c] ==
				...
				if(this.skin == "tdh_blue"){
					for(var i=0;i<a.parentNode.childNodes.length;i++){
						a.parentNode.childNodes[i].style.zIndex=a.parentNode.childNodes.length-i;
					}
					a.style.zIndex = a.parentNode.childNodes.length;
				}
				break
			}
		this._setRowSizes()
	...
	_getCoverLine: function() {
		if(!this._lineA) this._lineA = document.createElement("div"), this._lineA.className = "dhx_tablist_line", this._lineA.style[this._py] =
			this._s.line_height - 1 - (this._s.skin_line_x ? 1 : 0) + "px", this._lineA.style[this._dx] = "100%", this._lineA._index = 1;
	...
	cells: function(a) {
		...
		if(!this._content[a]) {
			...
			b.autoSize = function() {
				if(c._awdj || c._ahdj) {
					var a =
						this.vs ? this.vs[this.av].dhxcont.mainCont[this.av] : this.mainCont;
			...
			b._activate = function() {
				this._delay && this._attachContent.apply(this,
					this._delay);
				this.activate();
	...
	loadXMLString: function(a, b) {
		this.XMLLoader = new dtmlXMLLoaderObject(this._parseXML,
	...
	_parseXML: function(a, b, c, d, e) {
		...
		i && a.setStyle(i);
		a._hrfmode = g.getAttribute("hrefmode") || a._hrfmode;
		a._a.margin =
			parseInt(g.getAttribute("margin") || a._a.margin, 10);
		...
		for(var j = 0; j < h.length; j++)
			for(var k = e.doXPath("./tab", h[j]), l = 0; l < k.length; l++) {
				...
				a.addTab(m, q, o, "", j);
				k[l].getAttribute("selected") && (f = m);
				if(a._hrfmode) a.setContentHref(m,
	...
	setSkinColors: function(a, b, c) {
		...
		var e = document.styleSheets[document.styleSheets.length -
			1];
		e.addRule ? e.addRule("#" + this.entBox.id + " .dhx_tabcontent_zone > div", "background-color: " + (c || a) + ";", 0) : e.insertRule("#" + this.entBox.id + " .dhx_tabcontent_zone > div{ background-color: " + (c || a) + ";}", 0);
	...
	setCustomStyle: function(a, b, c, d) {
		...
		if(this._tabs[a]) this._tabs[a].childNodes[0].style.cssText = this._tabs[a] == this._lastActive ? this._c[a].scolor : this._c[a].color
	},
	...
	disableTab: function(a) {
		...
		this._tabs[a].className += " dhx_tab_element_disabled";
		this._tabs[a].style.color = "silver"
	},
	enableTab: function(a) {
		this._tabs[a]._disabled = !1;
		if(this._tabs[a].className) this._tabs[a].className = this._tabs[a].className.replace(/dhx_tab_element_disabled/g, "");
	...
	hideTab: function(a, b) {
		...
		c ==
			this._lastActive && b !== !1 && this.goToNextTab();
		this._setTabSizes(c.parentNode)
	},
	getActiveTab: function() {
		return !this._lastActive ? null : this._lastActive.getAttribute("tab_id")
	},
	...
	setAlign: function(a) {
		this._s.align = a == "bottom" || a == "right"
	},
	...
	normalize: function(a, b) {
		...
		this._lastActive = null;
		this._createRow();
		for(var h = 0, i = this._a.offset, j = Infinity, k = null, f = 0; f < d.length; f++) i + d[f]._size + this._a.margin < a ? (this._rows[h].appendChild(d[f]), this._rows[h].tabCount++, i += d[f]._size + this._a.margin) :
			(b && i < j && c(this, f - 1), this._createRow(), f--, h++, j = i, i = this._a.offset);
	...
	showInnerScroll: function() {
		for(var a in this._tabs) {
			...
			if(this.cells(a).vs) {
				...
				dhtmlx.CustomScroll.enable(this.cells(a).dhxcont.mainCont) : this.cells(a).dhxcont.mainCont.style.overflow = "auto"
		}
	},
