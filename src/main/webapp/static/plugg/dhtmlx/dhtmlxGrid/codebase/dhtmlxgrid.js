...
function dhtmlXGridObject(a) {
	...
	this.setSkin = function(e) {
		...
		switch(e) {
			case "clear":
				...
				this._topMb.innerHTML = "<img style='left:0px'   src='" + this.imgURL + "skinC_top_left.gif'><img style='right:20px' src='" +
	...
	this.init = function(e) {
		...
		if(this._hstyles) {
			...
			if(this.hdr.rows.length > 0) {
				var a = this.xmlFileUrl;
				this.clearAll(!0);
				this.xmlFileUrl = a
			}
			for(var b = this.hdr.insertRow(0), f = 0; f < this.hdrLabels.length; f++) b.appendChild(document.createElement("TH")), b.childNodes[f]._cellIndex = f, b.childNodes[f].style.height = "0px";
			_isIE && _isIE < 8 && document.body.style.msTouchAction == this.undefined ? b.style.position = "absolute" : b.style.height = "auto";
			b = this.hdr.insertRow(_isKHTML ? 2 : 1);
			b._childIndexes = [];
			for(var d = 0, f = 0; f < this.hdrLabels.length; f++) b._childIndexes[f] = f - d, this.hdrLabels[f] == this.splitSign && f != 0 ? (_isKHTML && b.insertCell(f - d), b.cells[f -
				d - 1].colSpan = (b.cells[f - d - 1].colSpan || 1) + 1, b.childNodes[f - d - 1]._cellIndex++, d++, b._childIndexes[f] = f - d) : (b.insertCell(f - d), b.childNodes[f - d]._cellIndex = f, b.childNodes[f - d]._cellIndexS = f, this.setColumnLabel(f, this.hdrLabels[f]));
			if(d == 0) b._childIndexes = null;
			...
			var h = this.obj.firstChild;
			if(!h.firstChild) {
				h.appendChild(document.createElement("TR"));
				h = h.firstChild;
				_isIE && _isIE < 8 && document.body.style.msTouchAction == this.undefined ? h.style.position = "absolute" : h.style.height = "auto";
				for(f = 0; f < this.hdrLabels.length; f++) h.appendChild(document.createElement("TH")), h.childNodes[f].style.height = "0px"
			}
			this._c_order = null;
			...
			this.noHeader ? this.hdrBox.style.display = "none" :
				this.noHeader = !1;
	...
	this.setSizes = function() {
		if(this.hdr.rows[0]) {
			var e = this.quirks = _isIE && document.compatMode == "BackCompat",
				a = (this.entBox.offsetWidth - this.entBox.clientWidth) / 2;
			if(!this.dontSetSizes)
				if(this.globalBox) {
					...
					if(this._delta_y && !this._realfake) this.globalBox.style.height = this._delta_y, this.entBox.style.overflow = this._fake.entBox.style.overflow = "hidden", this.entBox.style.height = this._fake.entBox.style.height = this.globalBox.clientHeight + (e ? b * 2 : 0) + "px"
				} else {
					if(this._delta_x) this.entBox.parentNode && this.entBox.parentNode.tagName ==
						"TD" ? (this.entBox.style.width = "1px", this.entBox.style.width = parseInt(this._delta_x) * this.entBox.parentNode.clientWidth / 100 - a * 2 + "px") : this.entBox.style.width = this._delta_x;
					if(this._delta_y) this.entBox.style.height = this._delta_y
				}
			window.clearTimeout(this._sizeTime);
			if(!this.entBox.offsetWidth && (!this.globalBox || !this.globalBox.offsetWidth)) this._sizeTime = window.setTimeout(function() {
				c.setSizes && c.setSizes()
			}, 250);
			else {
				var d = !this._wthB && (this.entBox.cmp || this._delta_x) && (this.skin_name || "").indexOf("dhx") ==
					0 && !e ? 2 : 0,
					h = !this._wthB && (this.entBox.cmp || this._delta_y) && (this.skin_name || "").indexOf("dhx") == 0 && !e ? 2 : 0;
				this._sizeFix && (d -= this._sizeFix, h -= this._sizeFix);
				var l = this.parentGrid ? !1 : this.objBox.scrollHeight > this.objBox.offsetHeight,
					k = dhtmlx.$customScroll ? 0 : 18,
					m = this.entBox.clientWidth - (this.skin_h_correction || 0) * (e ? 0 : 1) - d,
					n = this.entBox.clientWidth - (this.skin_h_correction || 0) - d,
					//o = this.entBox.clientHeight - h,
					o = this.entBox.style.height=="100%"?(this.entBox.parentNode.clientHeight - h - 5) : (this.entBox.clientHeight - h),
					p = this.setColumnSizes(n - (l ? k : 0) - (this._correction_x || 0)),
					r = this.parentGrid ? !1 : this.objBox.scrollWidth >
					this.objBox.offsetWidth || this.objBox.style.overflowX == "scroll",
					s = this.hdr.clientHeight,
					t = this.ftr ? this.ftr.clientHeight : 0,
					u = m,
					q = o - s - t;
				this._awdth && this._awdth[0] && this._awdth[1] == 99999 && (r = 0);
				if(this._ahgr && (q = this._ahgrMA ? this.entBox.parentNode.clientHeight - s - t : this.obj.offsetHeight + (r ? k : 0) + (this._correction_y || 0), this._ahgrM && (q = this._ahgrF ? Math.min(this._ahgrM, q + s + t) - s - t : Math.min(this._ahgrM, q)), l && q >= this.obj.scrollHeight + (r ? k : 0))) l = !1, this.setColumnSizes(n - (this._correction_x || 0));
				if(this._awdth &&
					this._awdth[0]) {
				...
				if(!this.dontSetSizes) this.entBox.style.width = Math.max(0, u + (e ? 2 : 0) * a + this._ff_size_delta) + "px", this.entBox.style.height = q + (e ? 2 : 0) * a + s + t + "px";
				this.objBox.style.height =
					q + (e && !l ? 2 : 0) * a - this.pageBoxHeight + "px";
				this.hdrBox.style.height = s + 2 + "px";
	...
	this.startColResize = function(a) {
		...
		b.tagName != "TD" && (b = this.getFirstParentOfType(b, "TD"));
		var i = a.clientX,
			f = this.hdr.offsetWidth,
			d = parseInt(b.offsetWidth);
		if(b.tagName ==
			"TD" && b.style.cursor != "default" && (!this._drsclmn || this._drsclmn[b._cellIndex])) c._old_d_mm = document.body.onmousemove, c._old_d_mu = document.body.onmouseup, document.body.onmousemove = function(a) {
			c && c.doColResize(a || window.event, b, d, i, f)
	...
	this.doColResize = function(a,
		...
		if(this.callEvent("onResize", [b._cellIndex, h, this])) {
			...
			} else this._setColumnSizeR(b._cellIndex, h);
			this.doOnScroll(0, 1);
			this.setSizes();
			this._fake && this._awdth &&
		...
		if(convertStringToBoolean(a)) {
			...
			if(d) {
				for(var h = 0; h < d.childNodes.length; h++)
					if(d.childNodes[h]._cellIndexS == b) return this.r_fldSorted = d.childNodes[h], this.setSortImgPos();
				return this.setSortImgState(a, b, c, (f || 1) + 1)
			}
		} else this.sortImg.style.display =
			"none", this.fldSorted = this.r_fldSorted = null
	};
	this.setSortImgPos = function(a, b, c, f) {
		if(!this._hrrar || !this._hrrar[this.r_fldSorted ? this.r_fldSorted._cellIndex : a])
			if(f || (f = a ? this.hdr.rows[c || 0].cells[a] : this.r_fldSorted), f != null) {
				var d = this.getPosition(f, this.hdrBox),
					h = f.offsetWidth;
				this.sortImg.style.left = Number(d[0] + h - 13) + "px";
				this.sortImg.defLeft = parseInt(this.sortImg.style.left);
				this.sortImg.style.top = Number(d[1] + 5) + "px";
				if(!this.useImagesInHeader && !b) this.sortImg.style.display = "inline";
				this.sortImg.style.left =
					this.sortImg.defLeft + "px"
			}
	};
		...
		if(a.button == 2 || _isMacOS && a.ctrlKey) {
			...
			if(this._ctmndx) {
				...
				if(this._ctmndx.showContextMenu) {
					...
					if(_isIE) var l = a.clientX + h[0],
						k = a.clientY + h[1];
					else l = a.pageX, k = a.pageY;
	...
	this.doClick = function(a, b, d, f) {
		...
		if(a.tagName == "TD") {
			...
			var l = this.row;
			if(d == 1) {
				var k = this.rowsCol._dhx_find(a.parentNode),
					m = this.rowsCol._dhx_find(this.lastClicked);
				if(k > m) var n = m,
					o = k;
				else n = k, o = m;
				for(var p = 0; p < this.rowsCol.length; p++)
					if(p >= n && p <= o)
						if(this.rowsCol[p] && !this.rowsCol[p]._sRow) this.rowsCol[p].className.indexOf("rowselected") == -1 && this.callEvent("onBeforeSelect", [this.rowsCol[p].idd, g]) && (this.rowsCol[p].className += " rowselected", this.selectedRows[this.selectedRows.length] = this.rowsCol[p]);
						else return this.clearSelection(), this.doClick(a, b, 0, f)
			...
			if(!r && !a.parentNode._sRow)
				if(this.callEvent("onBeforeSelect", [a.parentNode.idd, g]))
					if(this.getSelectedRowId() != a.parentNode.idd) {
						d == 0 && this.clearSelection();
						this.cell = a;
						l == a.parentNode && this._chRRS && (b = !1);
						this.row = a.parentNode;
						this.row.className += " rowselected"; /*if(this.cell&&_isIE&&_isIE==8){var s=this.cell.nextSibling,t=this.cell.parentNode;t.removeChild(this.cell);t.insertBefore(this.cell,s);this.cell.setActive()}*/
	...
	this.selectAll = function() {
		...
		if(this.selectedRows.length) this.row = this.selectedRows[0], this.cell = this.row.cells[0];
	...
	this.moveToVisible = function(a, b) {
		...
		try {
			...
			else var h = this._realfake ? this._fake.rowsAr[a.parentNode.idd] : a.parentNode,
				d = this.rowsBuffer._dhx_find(h) * this._srdh;
			var l = d + a.offsetHeight + 38;
			if(l > this.objBox.offsetHeight + this.objBox.scrollTop) var k = d;
			else d < this.objBox.scrollTop &&
				(k = d - 5);
	...
	this.editCell = function() {
		if(!(this.editor && this.cell == this.editor.cell)) {
			...
			if(this.editor != null) {
				...
				this.callEvent("onEditCell", [0, this.row.idd, this.cell._cellIndex]) != !1 && this.editor.edit ? (this._Opera_stop = (new Date).valueOf(), a.className += " editable", this.editor.edit(),
					this.callEvent("onEditCell", [1, this.row.idd, this.cell._cellIndex])) : this.editor = null
			}
		}
	};
	...
	this._getNextCell =
		function(a, b, c) {
			...
			return !a ? (b == 1 && this.tabEnd && (this.tabEnd.focus(), this.tabEnd.focus(), this.setActive(!1)), b == -1 && this.tabStart && (this.tabStart.focus(), this.tabStart.focus(), this.setActive(!1)),
				null) : a.style.display != "none" && (!this.smartTabOrder || !this.cells(a.parentNode.idd, a._cellIndex).isDisabled()) ? a : this._getNextCell(a, b)
		};
	...
	this.scrollPage = function(a) {
		if(this.rowsBuffer.length) {
			...
			c < 0 && (c = 0);
			c >= this.rowsBuffer.length && (c =
				this.rowsBuffer.length - 1);
	...
	this.doKey = function(a) {
		...
		if((a.target || a.srcElement).value !== window.undefined) {
			var b = a.target || a.srcElement;
			if(!b.parentNode || (b.className != "dhx_textarea" && b.parentNode.className.indexOf("editable") == -1)) return !0
		}
		...
		var c = "k" + a.keyCode + "_" + (a.ctrlKey ? 1 : 0) + "_" + (a.shiftKey ?
			1 : 0);
		if(this.cell) {
		...
		function(a, b) {
			if(a != b) {
				...
				for(var d = 0; d < c.childNodes.length; d++)
					if(c.childNodes[d]._code) c.childNodes[d]._code = this._compileSCL(c.childNodes[d]._val, c.childNodes[d]);
				if(this._mat_links && this._mat_links[a]) {
					var g = this._mat_links[a];
					delete this._mat_links[a];
					for(var h in g)
						for(d = 0; d < g[h].length; d++) this._compileSCL(g[h][d].original, g[h][d])
				}
	...
	this.getColLabel = this.getColumnLabel = function(a, b, c) {
		for(var d = (c || this.hdr).rows[(b || 0) + 1], g = 0; g <
			d.cells.length; g++)
			if(d.cells[g]._cellIndexS == a) return _isIE ? d.cells[g].innerText : d.cells[g].textContent;
		return ""
	};
	...
	this.setRowTextStyle = function(a, b) {
		...
		if(c)
			for(var d = 0; d < c.childNodes.length; d++) {
				...
				this._hrrar && this._hrrar[d] && (g = "display:none;");
	...
	this.setCellTextStyle = function(a, b, c) {
		...
		if(d) {
			...
			if(g) {
				var h = "";
				this._hrrar && this._hrrar[b] && (h = "display:none;");
	...
	this.clearSelection = function() {
		...
		for(var a = 0; a < this.selectedRows.length; a++) {
			var b =
				this.rowsAr[this.selectedRows[a].idd];
	...
	this.copyRowContent = function(a, b) {
		...
		if(!_isIE) this.getRowById(a).cells[0].height =
	...
	this.setColumnLabel = function(a, b, c, d) {
		var g = (d || this.hdr).rows[c || 1],
			h = g._childIndexes ? g._childIndexes[a] : a;
		if(g.cells[h]) {
	...
	this.sortField = function(a, b, c) {
		...
		if(d && d.tagName == "TH" && this.fldSort.length - 1 >= d._cellIndex && this.fldSort[d._cellIndex] != "na") {
			var g = this.getSortingState(),
				h = g[0] == a && g[1] == "asc" ? "des" : "asc";
			if(this.callEvent("onBeforeSorting", [a, this.fldSort[a], h])) {
				...
				this.fldSorted = d;
				this.r_fldSorted = c;
				var m = this.hdr.rows[1],
					m = c.parentNode,
					n = m._childIndexes ? m._childIndexes[d._cellIndex] : d._cellIndex;
	...
	this.enableHeaderImages = function(a) {
		this.useImagesInHeader =
			a
	};
	this.setHeader = function(a, b, c) {
		for(var d = typeof a != "object" ? this._eSplit(a) : [].concat(a), g = [], h = new dhtmlxArray(0), l = [], k = [], m = [], n = 0; n < d.length; n++) g[g.length] = Math.round(100 / d.length), h[h.length] = "ed", l[l.length] = "left", k[k.length] = "middle", m[m.length] = "na";
	...
	this.setStyle = function(a, b, c, d) {
		...
		for(var g = ["#" + this.entBox.id + " table.hdr td", "#" + this.entBox.id + " table.obj td", "#" + this.entBox.id + " table.obj tr.rowselected td.cellselected", "#" + this.entBox.id + " table.obj td.cellselected", "#" + this.entBox.id + " table.obj tr.rowselected td"], h = 0; !_isIE;) {
			try {
		...
		for(var m = 0; m < g.length; m++) this.ssModifier[m] && (_isIE ? document.styleSheets[0].addRule(g[m], this.ssModifier[m]) :
			document.styleSheets[h].insertRule(g[m] + (" { " + this.ssModifier[m] + " }"), document.styleSheets[h].cssRules.length))
	};
	...
	this.getChangedRows = function(a) {
		...
		this.forEachRow(function(c) {
			var d = this.rowsAr[c];
			if(d.tagName == "TR") {
				var g = d.childNodes.length;
				if(a && d._added) b[b.length] = d.idd;
				else
					for(var h = 0; h < g; h++)
						if(d.childNodes[h].wasChanged) {
							b[b.length] = d.idd;
							break
						}
			}
	...
	this.setSerializationLevel = function(a, b, c, d, g, h) {
		this._sUDa =
			a;
		this._sAll = b;
		this._sConfig = c;
	...
	this._serializeRow = function(a, b) {
		...
		if(d.length)
			for(b = 0; b <
				d.length; b++) c.push(" " + d[b] + "='" + a._attrs[d[b]] + "'");
		c.push(">");
		if(this._sUDa && this.UserData[a.idd]) {
			keysAr = this.UserData[a.idd].getKeys();
			for(var h = 0; h < keysAr.length; h++) c.push("<userdata name='" + keysAr[h] + "'>" + (this._asCDATA ? "<![CDATA[" : "") + this.UserData[a.idd].get(keysAr[h]) + (this._asCDATA ? "]]\>" : "") + "</userdata>")
		}
		for(var l = !1, k = 0; k < this._cCount; k++)
			if(!this._srClmn || this._srClmn[k]) {
				var m = this.cells3(a, k);
				c.push("<cell");
				if(g.length)
					for(b = 0; b < g.length; b++) c.push(" " + g[b] + "='" + m.cell._attrs[g[b]] +
						"'");
				zxVal = m[this._agetm]();
				this._asCDATA && (zxVal = "<![CDATA[" + zxVal + "]]\>");
				this._ecspn && m.cell.colSpan && m.cell.colSpan > 1 && c.push(' colspan="' + m.cell.colSpan + '" ');
				this._chAttr ? m.wasChanged() && (c.push(' changed="1"'), l = !0) : this._onlChAttr && m.wasChanged() && (l = !0);
				this._sAll && this.cellType[k] == "tree" ? c.push((this._h2 ? " image='" + this._h2.get[a.idd].image + "'" : "") + ">" + zxVal + "</cell>") : c.push(">" + zxVal + "</cell>");
				if(this._ecspn && m.cell.colSpan)
					for(var n = 0; n < m.cell.colSpan - 1; n++) c.push("<cell/>"), k++
			}
		return this._onlChAttr &&
			!l && !a._added ? "" : c.join("")
	};
	this._serialiseConfig = function() {
		for(var a = "<head>", b = 0; b < this.hdr.rows[0].cells.length; b++)
			if(!this._srClmn || this._srClmn[b]) {
				...
				a += this._asCDATA ?
					"<![CDATA[" + this.getHeaderCol(b) + "]]\>" : this.getHeaderCol(b);
				var d = this.getCombo(b);
				if(d)
					for(var g = 0; g < d.keys.length; g++) a += "<option value='" + d.keys[g] + "'>" + d.values[g] + "</option>";
				a += "</column>"
			}
		return a += "</head>"
	};
	this.serialize = function() {
		...
		if(this._sUDa && this.UserData.gridglobaluserdata)
			for(var b = this.UserData.gridglobaluserdata.getKeys(), c = 0; c < b.length; c++) a += "<userdata name='" + b[c] + "'>" + this.UserData.gridglobaluserdata.get(b[c]) +
				"</userdata>";
	...
	this.getPosition = function(a, b) {
		...
		b == document.body && (_isIE ? (h += document.body.offsetTop || document.documentElement.offsetTop, g += document.body.offsetLeft || document.documentElement.offsetLeft) : _isFF || (g += document.body.offsetLeft, h += document.body.offsetTop));
		return [g, h]
	};
	...
	if(!_isOpera || _OperaRv > 8.5) this.hdr.onmousemove =
		function(a) {
	...
	_isMacOS ? this.entBox.oncontextmenu = function(a) {
		...
		a.returnValue = !1;
		var b = this.grid;
		if(b._realfake) b = b._fake;
		return b._doContClick(a || window.event)
	...
	dhtmlxEvent(document.body, "click", function() {
		...
		c.isActive &&
			c.setActive(!1)
	}); /*this.entBox.onbeforeactivate=function(){this._still_active=null;this.grid.setActive();event.cancelBubble=!0};this.entBox.onbeforedeactivate=function(){this.grid._still_active?this.grid._still_active=null:this.grid.isActive=!1;event.cancelBubble=!0};*/
...
dhtmlXGridObject.prototype = {
	//分页
	enablePagingon: function(a,b,c){
		if(a){
			var that=this;
			this.pageTotal = 0; //总条数
			this.pageLimit = parseInt(b) || 10;//每页条数
			this.pageCount = 0;//页总数 
			this.pageStart = 1;//数据开始索引
			this.pageCurrent = 1; //当前页， 从1开始
			this.pageBox = document.createElement("DIV");
			this.pageBox.style.width = "100%";
			this.pageBox.style.overflow = "hidden";
			this.pageBox.className = "page";
			this.pageBox.innerHTML="<a id=\"pageprev\" class=\"page_prev\"></a>"+
									"<span class=\"page_span\">第"+this.pageCurrent+"页，共"+this.pageCount+"页</span>"+
									"<a id=\"pagenext\" class=\"page_next\"></a>"+
									"<span class=\"page_span\">跳转至</span>"+
									"<input class=\"page_to\" type=\"text\" />"+
									"<span class=\"page_span\">页</span>"+
									"<a id=\"pageto\" class=\"page_btn\">确定</a>"+
									"<span class=\"page_toggle\">共"+this.pageTotal+"条</span>";
			this.entBox.appendChild(this.pageBox);
			this.pageBoxHeight = this.pageBox.clientHeight;
			this.pageFlag=true;
			this.pageNumObj = this.pageBox.children[1];
			this.pageToObj = this.pageBox.children[4];
			this.pageTotalObj = this.pageBox.children[7];
			
			this.doPageEvent();
		}
	},
	loadPage:function(type){
		this.pageStart = (this.pageCurrent - 1) * this.pageLimit+1;
		var newPageUrl = '';
		if (this.pageUrl.indexOf("?") >= 0) {
			newPageUrl = this.pageUrl + "&start=" + this.pageStart + "&limit=" + this.pageLimit;
		} else {
			newPageUrl = this.pageUrl + "?start=" + this.pageStart + "&limit=" + this.pageLimit;
		}
		this.clearAll();
		var that=this;
		this.load(newPageUrl, function(){
			if(type){
				that.pageTotal = that.getUserData("","totalnumber") || 0;
			}
			that.doChangePage();
		}, "xml");
	},
	doChangePage:function(){
		this.pageCount = Math.ceil(this.pageTotal / this.pageLimit);
		this.pageNumObj.innerHTML = "第"+this.pageCurrent+"页，共"+this.pageCount+"页";
		this.pageTotalObj.innerHTML = "共"+this.pageTotal+"条";
		if(typeof(this.pageCallback) == "function"){
			this.pageCallback();
		}
	},
	doPageEvent:function(){
		var that = this;
		that.pageBox.onclick=function(a){
			...
			if(d.id=='pageprev'){
				...
				var pageToVal = that.pageToObj.value;
				if(!!pageToVal){
					if(isNaN(pageToVal)){
						layer.alert("页码不正确  请确认",{title:"错误提示消息"},function(index){
							layer.close(index);
							that.pageToObj.focus();
						});
						return ;
					}else if(parseInt(pageToVal)>that.pageCount || parseInt(pageToVal)<1){
						layer.alert("没有该页数据！请确认输入的页码",{title:"错误提示消息"},function(index){
							layer.close(index);
							that.pageToObj.focus();
						}); 
						return ;
					}
					that.pageCurrent = parseInt(pageToVal);
				}else{
					return false;
				}
			}else{
		...
		if(d) {
			...
			if(b && d.style.display != "none") {
				...
				if(e != -1) {
					...
					for(var j =
							0; j < d.childNodes.length; j++) d.childNodes[j].className = d.childNodes[j].className.replace(/cellselected/g, "");
	...
	_fixHiddenRowsAll: function(a, b, c, d, e) {
		for(var e = e || "_cellIndex", j = a.rows.length, i = 0; i < j; i++) {
			var f = a.rows[i].childNodes;
			if(f.length != this._cCount)
	...
	_parseHead: function(a) {
		...
		if(b.length) {
			...
			if(d[0])
				for(var i =
					...
					case "colwidth":
						...
						break;
					case "splitat":
						j = d[0].childNodes[i].firstChild ? d[0].childNodes[i].firstChild.data : !1
				}
			...
			if(c.length > 0) {
				...
				for(var f = [
						...
						[],
						[]
					], g = ",width,type,align,sort,color,format,hidden,id".split(","), h = ["", e,
				...
				for(l = 0; l < h.length; l++)
					if(h[l]) this[h[l]](f[l].join(this.delim));
					...
					if(this.cellType[l].indexOf("co") == 0 || this.cellType[l] == "clist") {
						...
						if(m.length) {
							...
							if(this.cellType[l] ==
								...
								for(var o = this.getCombo(l), k = 0; k < m.length; k++) o.put(m[k].getAttribute("value"), m[k].firstChild ? m[k].firstChild.data : "")
						}
		...
		if(r.length > 0) {
			...
			for(k = 0; k < r.length; k++) {
				...
				this.UserData.gridglobaluserdata.put(r[k].getAttribute("name"), s)
			}
		}
	},
			...
			if(d._childIndexes && d.childNodes[d._childIndexes[b]]) {
				...
				if(i && i != 1)
					for(var f = 1; f < i; f++) {
						...
						j.nextSibling ? d.insertBefore(g, j.nextSibling) : d.appendChild(g);
						d._childIndexes[b + f] = e + f;
						g._cellIndex = b + f;
						g.style.textAlign = this.cellAlign[f];
						g.style.verticalAlign = this.cellVAlign[f];
						j = g;
						this.cells3(d, b + f).setValue("")
					}
				for(var h = b * 1 + 1 * i; h < d._childIndexes.length; h++) d._childIndexes[h] += (i - 1) * 1
			}
			if(c && c > 1) {
				if(d._childIndexes) e = d._childIndexes[b];
					...
					e = b;
					d._childIndexes = [];
					for(h = 0; h < d.childNodes.length; h++) d._childIndexes[h] =
						h
				}
				d.childNodes[e].colSpan = c;
				for(h = 1; h < c; h++) d._childIndexes[d.childNodes[e + 1]._cellIndex] = e, d.removeChild(d.childNodes[e + 1]);
				for(var l = d.childNodes[d._childIndexes[b]]._cellIndex, h = l * 1 + 1 * c; h < d._childIndexes.length; h++) d._childIndexes[h] -= c - 1
			}
		}
	},
	...
	adjustColumnSize: function(a, b) {
		if(!this._hrrar || !this._hrrar[a]) {
			...
			for(var d = 1; d < this.hdr.rows.length; d++) {
				...
				if((e = e.childNodes[e._childIndexes ? e._childIndexes[a] : a]) && (!e.colSpan || e.colSpan < 2) && e._cellIndex == a) e.childNodes[0] && e.childNodes[0].className == "hdrcell" && (e = e.childNodes[0]), c = Math.max(c, e.scrollWidth)
			}
			for(var j = this.obj.rows.length, i = 1; i < j; i++) {
				var f =
					this.obj.rows[i];
	...
	_createHRow: function(a, b) {
		if(!b) {
			...
			this.ftr = b = d;
			for(var e = d.insertRow(0), j = this.hdrLabels.length <= 1 ? a[0].length : this.hdrLabels.length, i = 0; i < j; i++) e.appendChild(document.createElement("TH")), e.childNodes[i]._cellIndex = i;
		...
		for(i = 0; i < a[0].length; i++)
			...
			for(var h = b.rows.length - 2, l = !1, g = null; !l;) {
				for(var g = b.rows[h], k = 0; k < g.cells.length; k++)
					if(g.cells[k]._cellIndex == i) {
						l = k + 1;
						break
					}
				h--
			...
			m._cellIndex = m._cellIndexS = i;
			if(this._hrrar && this._hrrar[i] && !_isIE) m.style.display = "none";
			if(typeof a[0][i] ==
				"object") m.appendChild(a[0][i]);
			else if(m.innerHTML = this.forceDivInHeader ? "<div class='hdrcell'>" + (a[0][i] || "&nbsp;") + "</div>" : a[0][i] || "&nbsp;", (a[0][i] || "").indexOf("#") != -1)
				if(d = a[0][i].match(/(^|{)#([^}]+)(}|$)/)) {
					var n = "_in_header_" + d[2];
					if(this[n]) this[n](this.forceDivInHeader ? m.firstChild : m, i, a[0][i].split(d[0]))
				}
			if(f) m.style.cssText = f[i];
			c.appendChild(m)
		}
		var o = b;
	...
	setColumnExcellType: function(a, b) {
		for(var c = 0; c < this.rowsBuffer.length; c++) this.rowsBuffer[c] &&
			this.rowsBuffer[c].tagName == "TR" && this.changeCellType(this.rowsBuffer[c], a, b);
	...
	_refreshFromXML: function(a) {
		...
		for(var e = a.doXPath("//row"), j = 0; j < e.length; j++) {
			var i = e[j],
				f = i.getAttribute("id");
			d[f] = !1;
			c = i.parentNode.getAttribute("id") || c;
			if(this.rowsAr[f] && this.rowsAr[f].tagName != "TR") this._h2 ? this._h2.get[f].buff.data =
				i : this.rowsBuffer[this.getRowIndex(f)].data = i, this.rowsAr[f] = i;
			else if(this.rowsAr[f]) this._process_xml_row(this.rowsAr[f], i, -1), this._postRowProcessing(this.rowsAr[f], !0);
				...
				if(this._h2) reset = !0, this._h2.add(f, i.parentNode.getAttribute("id") || i.parentNode.getAttribute("parent")).buff =
					this.rowsBuffer[this.rowsBuffer.length - 1];
				this.rowsAr[f] = i;
				i = this.render_row(h);
				this._insertRowAt(i, h ? -1 : 0)
			}
		}
		if(this._refresh_mode[2])
			for(f in d) d[f] && this.rowsAr[f] && this.deleteRow(f);
	...
	_key_events: {
		...
		k13_0_1: function() {
			var a =
				this.rowsCol._dhx_find(this.row);
			this.selectCell(this.rowsCol[a - 1], this.cell._cellIndex, !0)
		},
		k13_0_0: function() {
			this.editStop();
		...
		k40_0_0: function() {
			...
			if(this.editor && this.editor.combo) this.editor.shiftNext();
				...
				if(c) a._r_select = null, this.selectCell(c, this.cell._cellIndex, !0), a.pagingOn && a.showRow(c.idd);
					...
					this.pagingOn && this.rowsCol[b + 1] && this.selectCell(b + 1, 0, !0)
				}
			}
			this._still_active = !0
		},
	...
	_prepareRow: function(a) {
		...
		for(var b = this._master_row.cloneNode(!0), c = 0; c < b.childNodes.length; c++) {
			...
			this.dragAndDropOff && this.dragger.addDraggableItem(b.childNodes[c],
				this)
		}
		b.idd = a;
		b.grid = this;
	...
	_process_xml_row: function(a, b) {
		...
		if(this._ud_enabled)
			for(var e = this.xmlLoader.doXPath("./userdata", b), j = e.length - 1; j >= 0; j--) {
				for(var i = "", f = 0; f < e[j].childNodes.length; f++) i += e[j].childNodes[f].nodeValue;
				this.setUserData(a.idd, e[j].getAttribute("name"), i)
			}
		for(f = 0; f < c.length; f++) {
			var g = c[this._c_order ? this._c_order[f] : f];
			if(g) {
				var h = a._childIndexes ? a._childIndexes[f] : f,
					l = g.getAttribute("type");
				if(a.childNodes[h]) {
					if(l) a.childNodes[h]._cellType = l;
					a.childNodes[h]._attrs = this._xml_attrs(g)
				}
		...
		for(var c = 0; c < a.childNodes.length; c++)
			if(c < b.length || this.defVal[c]) {
				var d = a.childNodes[c]._cellIndex,
					e = b[d],
					j = this.cells4(a.childNodes[c]);
				if(this.defVal[d] && (e == "" || typeof e == "undefined")) e = this.defVal[d];
				j && j.setValue(e)
			} else a.childNodes[c].innerHTML = "&nbsp;",
				a.childNodes[c]._clearCell = !0;
		return a
	},
	_postRowProcessing: function(a, b) {
		...
		for(var c = 0, d = 0; d < a.childNodes.length; d++) {
			...
			if(i = e._attrs.align || this.cellAlign[j]) e.align = i;
			e.vAlign = e._attrs.valign || this.cellVAlign[j];
			var f =
				e._attrs.bgColor || this.columnColor[j];
			if(f) e.bgColor = f;
			e._attrs.colspan && !b && (this.setColspan(a.idd, d + c, e._attrs.colspan), c += e._attrs.colspan - 1);
			if(this._hrrar && this._hrrar[j] && !b) e.style.display = "none"
		}
	...
	load: function(a, b, c) {
		...
		this._data_type = c;
		this.xmlLoader.onloadAction = function(a, e, j, i, f) {
			a.callEvent && (f = a["_process_" +
				c](f), a._contextCallTimer || a.callEvent("onXLE", [a, 0, 0, f]), b && (b(), b = null))
		};
	...
	_process_xml: function(a) {
		...
		if(c.tagName == this.xml.top) {
			...
			var e = a.doXPath(this.xml.row, c),
				j = parseInt(a.doXPath("//" + this.xml.top)[0].getAttribute("pos") || 0),
				i = parseInt(a.doXPath("//" + this.xml.top)[0].getAttribute("total_count") || 0),
				f = !1;
			i && i != this.rowsBuffer.length && (this.rowsBuffer[i - 1] || (this.rowsBuffer.length && (f = !0), this.rowsBuffer[i - 1] = null), i < this.rowsBuffer.length && (this.rowsBuffer.splice(i, this.rowsBuffer.length - i), f = !0));
			if(this.isTreeGrid()) return this._process_tree_xml(a);
			for(var g = 0; g < e.length; g++)
				if(!this.rowsBuffer[g + j]) {
					var h = e[g].getAttribute("id") || g + j + 1;
					this.rowsBuffer[g + j] = {
						idd: h,
						data: e[g],
						_parser: this._process_xml_row,
	...
	_process_jsarray: function(a) {
		this._parsing = !0;
		if(a && a.xmlDoc) eval("dhtmlx.temp=" +
			a.xmlDoc.responseText + ";"), a = dhtmlx.temp;
		for(var b = 0; b < a.length; b++) {
			var c = b + 1;
	...
	render_dataset: function(a, b) {
		...
		for(var c = a; c < b; c++) {
			...
			if(d == -1) {
				if(this.xmlFileUrl && this.callEvent("onDynXLS", [c,
						this._dpref ? this._dpref : b - c
					])) this.load(this.xmlFileUrl + getUrlSymbol(this.xmlFileUrl) + "posStart=" + c + "&count=" + (this._dpref ? this._dpref : b - c), this._data_type);
				b = c;
				break
			}
		...
		if(this._srnd && !this._fillers) {
			var j = this.rowsBuffer.length - b;
			for(this._fillers = []; j > 0;) {
				var i = _isIE || window._FFrv ? Math.min(j, 5E4) : j,
					f = this._add_filler(b, i);
	...
	sortRows: function(a, b, c) {
		...
		b = b || this.fldSort[a];
		a = a || 0;
		if(this.isTreeGrid()) this.sortTreeRows(a, b, c);
		else {
			var d = {},
				e = this.cellType[a],
				j = "getValue";
			e == "link" && (j = "getContent");
			if(e == "dhxCalendar" || e == "dhxCalendarA") j = "getDate";
			for(var i = 0; i < this.rowsBuffer.length; i++) d[this.rowsBuffer[i].idd] = this._get_cell_value(this.rowsBuffer[i], a, j);
	...
	_sortCore: function(a, b, c, d, e) {
		var j = "sort";
		if(this._sst) e.stablesort = this.rowsCol.stablesort, j = "stablesort";
		b.length > 4 && (b = window[b]);
		if(b == "cus") {
			var i =
				this._customSorts[a];
			e[j](function(a, b) {
				return i(d[a.idd], d[b.idd], c, a.idd, b.idd)
			})
		} else if(typeof b == "function") e[j](function(a, e) {
			return b(d[a.idd], d[e.idd], c, a.idd, e.idd)
		});
		else if(b == "str") e[j](function(a, b) {
			return c == "asc" ? d[a.idd] > d[b.idd] ? 1 : -1 : d[a.idd] < d[b.idd] ? 1 : -1
		});
		else if(b == "int") e[j](function(a, b) {
		...
		else if(b == "date") e[j](function(a, b) {
			var e = Date.parse(d[a.idd]) ||
				Date.parse("01/01/1900"),
				i = Date.parse(d[b.idd]) || Date.parse("01/01/1900");
			return c == "asc" ? e - i : i - e
		})
	},
	...
	_reset_view: function(a) {
		if(this.obj.rows[0]) {
			this._lahRw && this._unsetRowHover(0, !0);
			this.callEvent("onResetView", []);
			var b = this.obj.rows[0].parentNode,
				c = b.removeChild(b.childNodes[0], !0);
			if(_isKHTML)
				for(var d = b.parentNode.childNodes.length - 1; d >= 0; d--) b.parentNode.childNodes[d].tagName ==
					"TR" && b.parentNode.removeChild(b.parentNode.childNodes[d], !0);
			else if(_isIE)
	...
	deleteRow: function(a, b) {
		...
		if(b) {
			...
			var c = 0;
			if(this.cellType._dhx_find("tree") != -1 && !this._realfake) c =
				this._h2.get[a].parent.id, this._removeTrGrRow(b);
			else {
			...
			if(this._srnd) {
				for(e = 0; e < this._fillers.length; e++) {
					var j = this._fillers[e];
					j && this._update_fillers(e,
						j[1] >= d ? -1 : 0, j[0] >= d ? -1 : 0)
				}
	...
	_addRow: function(a, b, c) {
		...
		this.rowsAr[d.idd] = d;
		if(this._h2) this._h2.get[d.idd].buff =
			d;
		this._fillRow(d, b);
		...
		if(this._fillers) {
			...
			this.rowsAr[d.idd] = d;
			for(var j = !1, i = 0; i < this._fillers.length; i++) {
				var f = this._fillers[i];
				if(f && f[0] <= c && f[0] + f[1] >= c) {
					f[1] += 1;
					var g = f[2].firstChild.style.height = parseInt(f[2].firstChild.style.height) +
						this._srdh + "px",
						j = !0;
					if(this._fake) this._fake._fillers[i][1]++, this._fake._fillers[i][2].firstChild.style.height = g
				}
				f && f[0] > c && (f[0] += 1, this._fake && this._fake._fillers[i][0]++)
			}
			j || this._fillers.push(this._add_filler(c, 1, c == 0 ? {
	...
	_insertRowAt: function(a, b, c) {
		...
		if(this._cssEven) {
			...
			a.className += (this._cssSP ? this.getLevel(a.idd) : b) % 2 == 1 ? d + " " + this._cssUnEven +
				(this._cssSU ? " " + this._cssUnEven + "_" + this.getLevel(a.idd) : "") : d + " " + this._cssEven + (this._cssSU ? " " + this._cssEven + "_" + this.getLevel(a.idd) : "")
		}
			...
			if(b.tagName != "TR") {
				for(var c = 0; c < this.rowsBuffer.length; c++)
					if(this.rowsBuffer[c] &&
						this.rowsBuffer[c].idd == a) return this.render_row(c);
	...
	cells: function(a, b) {
		...
		!d && c._childIndexes && (d = c.firstChild || {});
	...
	setExternalTabOrder: function(a, b) {
		...
		this.tabEnd.onkeydown = this.tabEnd.onkeypress = function(a) {
			...
			var b = a || window.event;
			if(b.keyCode ==
				9 && b.shiftKey) return b.cancelBubble = !0, c.selectCell(c.getRowsNum() - 1, c.getColumnCount() - 1, 0, 0, 1), c.smartTabOrder && c.cells2(c.getRowsNum() - 1, c.getColumnCount() - 1).isDisabled() && c._key_events.k9_0_1.call(c), this.blur(), !1
		};
...
(function() {
	...
	function e(a, b) {
		...
		for(var d = 1, e = [], k = 0; k < b.length; k++)
			if(typeof b[k] == "object" && b[k].length)
				for(var j = 0; j < b[k].length; j++) c(k, j, b[k][j]);
			else c(k,
				0, b[k]);
		for(k = 0; k < e.length; k++)
			for(j = 0; j < e[0].length; j++) {
				...
				e[k][j] = (n || "").toString() || "&nbsp;";
				if(n && n.colspan)
					for(var o = 1; o < n.colspan; o++) c(j + o, k, "#cspan");
				if(n && n.rowspan)
					for(o = 1; o < n.rowspan; o++) c(j, k + o, "#rspan")
			}
		this.setHeader(e[0]);
	...
	dhtmlx.extend_api("dhtmlXGridObject", {
		...
		columns: function(a) {
			for(var c = 0; c < j.length; c++) {
				for(var d = [], e = 0; e < a.length; e++) d[e] = a[e][j[c].name] || j[c].def;
				var l = j[c].type || b;
				l.call(this, j[c].operation, d, a)
			}
			this.init()
		},
		rows: function() {},
...
dhtmlXGridObject.prototype._dp_init = function(a) {
	...
	a._getRowData = function(a) {
		...
		for(var d = this.obj.getRowById(a), e = 0; e < this.obj._cCount; e++) {
			var j = this.obj._c_order ? this.obj._c_order[e] : e,
				i = this.obj.cells(d.idd, e);
			if(!this._changed || i.wasChanged()) this._endnm ? c[this.obj.getColumnId(e)] = i.getValue() : c["c" + j] = i.getValue()
		}
		var f = this.obj.UserData[a];
		if(f)
			for(var g = 0; g < f.keys.length; g++) f.keys[g] && f.keys[g].indexOf("__") != 0 && (c[f.keys[g]] = f.values[g]);
		if(f = this.obj.UserData.gridglobaluserdata)
			for(g = 0; g < f.keys.length; g++) c[f.keys[g]] = f.values[g];
		return c
	};
	a._clearUpdateFlag = function(a) {
		var c =
			this.obj.getRowById(a);
		if(c)
	...
	a.checkBeforeUpdate = function(a) {
		for(var c = !0, d = [], e = 0; e < this.obj._cCount; e++)
			if(this.mandatoryFields[e]) {
				var j = this.mandatoryFields[e].call(this.obj, this.obj.cells(a, e).getValue(), a, e);
				typeof j == "string" ? (this.messages.push(j), c = !1) : (c &= j, d[e] = !j)
			}
