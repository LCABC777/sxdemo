...
*/
function serialize(formSelector,decode,prefix){
    if(decode == undefined) decode = false;
    if(prefix == undefined) prefix ="";
    if(decode){
        return reParamsSentence(searchForPms(formSelector,prefix),true);
    }else{
...
function formSetData(formSelector,jsondata,prefix,callback){
	 ...
	 $.each(jsondata,function(name,value){
	         if(prefix==''){
	   	  	 	form.find("[name='"+name+"']").val(escape2Html(value));
	   	  	 }else{
	   	  	 	form.find("[name='"+prefix+name+"']").val(escape2Html(value));
	   	  	 }
	 });
	 //回调函数
	 if(callback!=undefined){
...
function searchForPms(selector,prefix) {
    ...
    obj.find(":input:not(button,select,:submit,:image,:reset,:button,:file,:radio,:checkbox)")
        .each(function () {
            	...
            	if (!!$.trim($(this).val())) {
	                if(pms[prefix+ $(this).attr("name")]){
	                  if(pms[prefix+ $(this).attr("name")]!=''){
	                	...
	                	var fmt = $(this).attr("tdh-serifmt");
	                	if(fmt){
	                		if(fmt == "rq"){
	                			pms[prefix+ $(this).attr("name")] = $.trim($(this).val()).replace(new RegExp('-','gm'), '');
	                		}
	                	}else{
    ...
    obj.find(":checkbox:checked").each(function () {
            if ($.trim($(this).attr("name")) != '') {
                if(!pms[prefix+$(this).attr("name")]){
                	...
                	pms[prefix+$(this).attr("name")] = pms[prefix+$(this).attr("name")] + "," + $.trim($(this).val()); 
                }
            }
        });
    // 查找块标签内的 radio
    	...
    	if ($.trim($(this).attr("name")) != '') {
    		if(!!$.trim($(this).val())){
		        if(pms[prefix+ $(this).attr("name")]){ 
		            ...
		            if($(this).attr("multiple")){  //多选赋值
		                if($.trim($(this).val())){
		                    var $v = $.trim($(this).val());
		                    if(($v.indexOf('09_') > -1 || $v.indexOf('15_') > -1) && $v.indexOf('-') > -1){
		                        var $k = $v.split("-")[0] + '-';
		                        $v = $k + $v.replace(new RegExp($k,'gm'), '');
		                    }
		                    pms[prefix+ $(this).attr("name")] = $v;
		                }else{
 ...
 */
/**
 * jq(":input[name=id],[name=password],[name=name]").val('');
